...
- **USB2CAN SLCAN** (WeAct USB2CANFDV2 등): 드라이버 불필요 — USB CDC 가상 COM 포트로 동작.
  ...
  그 사실을 상태줄에 알립니다(설정은 바꾸지 않음 — 다음 ValueCAN 세션이 반쪽이 되지 않도록). 클래식과 CAN FD 모두 지원하고,
  **속도는 프리셋 목록에 매이지 않습니다** — 요청한 값에서 장치의 비트 타이밍(60 MHz 클록 분주 + 세그먼트,
  샘플포인트 80%)을 계산해 설정하므로, 프리셋에 없는 조합도 그대로 들어갑니다.
  예: `Bus ▸ Channels…`에 `CAN1@500000:500000`이면 중재 500k · 데이터 500k인 CAN FD.
  중재는 최대 1M(클래식 CAN 규격 상한), 데이터는 최대 5M(보드의 절연 트랜시버 정격)이며,
  60 MHz로 정수 분주가 안 되는 값은 가능한 이웃 값을 알려 주며 거절합니다. 하드웨어 타임스탬프가 없어 프레임 시각은 호스트 수신 시각이며,
  TX 에코는 펌웨어가 "버스에 실제로 나갔다"고 확인(CR ACK)한 시점에 표시됩니다. **송신은 그 확인을
  기다렸다가 반환**하므로(정상 버스에서 수 ms, 최대 250 ms) 상대가 없는 버스에서 보내면 화면뿐 아니라
  **TCP API·python·MCP 클라이언트도 실패를 오류로 받습니다** — 화면을 보지 않는 쪽에 성공이라고
  답하지 않기 위해서입니다.
...
> **Connect는 새 캡처를 시작합니다** — 직전 세션이 남긴 프레임은 버려지고 몇 개를 버렸는지 상태줄에
> 알립니다. ValueCAN 말고는 모두 열 때마다 시계가 0에서 다시 시작해서, 남겨 두면 버퍼의 시간이
> 거꾸로 흐르고 `recent()`가 지금 장치를 물어봤는데 이전 장치(가상 버스라면 지어낸) 트래픽을 답하게 됩니다.
...
로그를 열면 화면은 **비어 있고 위치는 0**입니다. Play를 눌러야 파일의 시간 순서대로 흘러갑니다.
통째로 올려놓고 훑는 것과 다른 이유는, 뷰가 말하는 것들 대부분이 **시계가 움직여야 의미가
생기기 때문**입니다 — Fixed 뷰의 Count가 올라가고, 주기가 자리를 잡고, 값이 바뀌는 바이트가
*바뀌는 순간에* 반짝입니다. 다 올려놓으면 셋 다 이미 끝난 상태로만 보입니다.

| 컨트롤 | |
|---|---|
| Play / Pause | `F7`. 라이브의 Pause와 같은 키 — 어느 쪽이든 "화면을 멈춘다"는 뜻입니다 |
| ⏮ | 파일 처음으로 |
| 배속 | 0.1x ~ 100x, 그리고 **Max**(그릴 수 있는 만큼) |
| 탐색 슬라이더 | 드래그 후 **놓을 때** 반영. 뒤로 가면 처음부터 다시 재생해서 집계를 맞추므로 큰 파일에서는 몇 초 걸립니다 |

**변경 강조는 재생 시계 기준으로 사라집니다.** 벽시계 기준이면 10배속에서 10초치 변경이 1초
안에 도착해 전부 동시에 켜졌다 꺼집니다.

한 틱이 반영할 수 있는 파일 시간은 **0.25초로 제한**됩니다. UI 스레드가 잠깐 멈추면(GC, 대화상자,
바쁜 머신) 그 시간이 한꺼번에 반영되어 재생이 순간이동하는데, 실제로 그 증상을 관측했습니다.
제한하면 "재생이 잠깐 느려졌다"로 나타나고, 그건 사실이면서 무해합니다.

실측: 1x가 벽시계를 따라가고(1초 → 0.996~1.009 s, 1,080행 ≈ 실제 버스 속도),
Max로 480초 파일을 약 14.6초에 재생합니다(약 33배).
...
- Connect 버튼이 `Close log`로 바뀌고, Connect·Clear·TX가 비활성 (Pause는 재생 제어로 살아납니다)
- 툴바 위에 앰버색 전송 바가 나타납니다

Clear를 막는 이유는 반사적으로 눌리는 버튼이라 수 초 걸린 로드를 되돌릴 방법 없이 날리기
때문입니다. 로그를 벗어나는 길은 `Close log` 하나입니다.
...
| 패널에 행 만들기 | 약 2.5 s / 패널 (UI 스레드, 대기 커서), 약 133 MiB — 재생 중에는 틱마다 나눠 지불합니다 |
| Max 배속 전체 재생 | 480 s 파일 약 14.6 s |
